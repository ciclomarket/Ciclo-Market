...
function ListingsSection({ listings, onRefresh, userId }: { listings: Listing[], onRefresh: () => void, userId: string }) {
  ...
  const [igCardModal, setIgCardModal] = useState<{ url: string; caption: string; listingId: string } | null>(null)
  ...
  const handleGenerateIgCard = async (listingId: string) => {
    ...
    try {
      ...
      if (result.ok) {
        setIgCardModal({ url: result.data.url, caption: result.data.caption ?? '', listingId })
      } else {
        ...
        >
          ...
          >
            <div className="sticky top-0 z-10 flex items-center justify-between bg-gradient-to-r from-[#833ab4] via-[#fd1d1d] to-[#fcb045] p-4">
              ...
              <button type="button" aria-label="Cerrar" className="rounded-full p-1 text-white/80 hover:text-white" onClick={() => setIgCardModal(null)}>
                <X className="h-5 w-5" />
              </button>
            </div>

            <div className="p-4 pb-2">
              <img src={igCardModal.url} alt="Post de Instagram" className="w-full rounded-xl border border-gray-100 object-cover" style={{ aspectRatio: '1080/1350' }} />
              <div className="mt-2 flex items-center justify-between">
                <span className="text-xs text-gray-400">1080×1350px · PNG</span>
                <a
              ...
              <div className="px-4 pb-2">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button
                    ...
                    onClick={() => {
                      navigator.clipboard.writeText(igCardModal.caption).then(() => showToast('Caption copiado'))
                        .catch(() => showToast('No se pudo copiar', { variant: 'error' }))
                    }}
                  >
                ...
                <pre className="whitespace-pre-wrap rounded-xl border border-gray-100 bg-gray-50 p-3 text-xs text-gray-700 leading-relaxed max-h-48 overflow-y-auto font-sans">{igCardModal.caption}</pre>
              </div>
            )}
            ...
            <div className="flex gap-2 border-t border-gray-100 p-4">
              <button
                ...
                onClick={() => {
                  navigator.clipboard.writeText(igCardModal.url).then(() => showToast('Enlace copiado'))
                    .catch(() => showToast('No se pudo copiar el enlace', { variant: 'error' }))
                }}
              >
